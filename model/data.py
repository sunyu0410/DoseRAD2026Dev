...
class PlanData(Dataset):
    ...
    def rotate_dose(self):
        ...
        for beam_idx, dose in enumerate(self.doses):
            angles = torch.tensor([c['ga'] for c in plan.cp[beam_idx].values()]).cuda()

            input_tensor = torch.randn(dose[:30].shape).to(torch.float16).cuda()

            for i in range(6):
                input_tensor[:] = dose[30*i:(i+1)*30]
                d_rot = rotate_image_batched(
                    input_tensor,  # Shape: (D, H, W) or (1, 1, D, H, W) on GPU
...
#     d.cuda(),  # Shape: (D, H, W) or (1, 1, D, H, W) on GPU

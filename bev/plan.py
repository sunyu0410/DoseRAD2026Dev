...
class Plan:
    ...
    def set_beam_id(self, n):
        assert n <= self.n_beams
        self.beam_id = n

    def set_cp_idx(self, n):
        assert n <= len(self.cp[self.beam_idx])
        sefl.cp_idx = n

    def generate_view(self):
        beam = self.beam_info[self.beam_idx]
        cp = self.cp[self.beam_idx][self.cp_idx]
        ...
        #     isocentre=isocentre,
        #     degree=gantry_angle,
        #     outfile=f"data/class/ct_b{self.beam_idx}_cp{self.cp_idx}.nii.gz",
        # )

        # rotate_image(
        #     infile=f"data/Dose_B0_CP{self.cp_idx:03d}.mha",
        #     isocentre=isocentre,
        #     degree=gantry_angle,
        #     outfile=f"data/class/dose_b{self.beam_idx}_cp{self.cp_idx}.nii.gz",
        #     bg_value=0,
        # )
        ...
        # sitk.WriteImage(img, f"data/class/bev_b{self.beam_idx}_cp{self.cp_idx}.nii.gz")
